...
class ResPartner(models.Model):
    ...
    claim_count = fields.Integer(string='# Reclamações', compute='_compute_claim_count')

    @api.model
    def _compute_claim_count(self):
        partners = self | self.mapped('child_ids')
        partner_data = self.env['crm.claim'].read_group([('partner_id', 'in', partners.ids)], ['partner_id'],
            ['partner_id'])
        mapped_data = dict(
